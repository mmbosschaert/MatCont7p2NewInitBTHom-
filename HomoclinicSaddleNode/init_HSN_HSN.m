function [x,v] = init_HSN_HSN(odefile, x, v, s, p, ap, ntst, ncol,extravec,T,eps0,eps1)

global homds cds
if strcmp(func2str(cds.curve),func2str(@homoclinic))
    [x,v] = init_Hom_HSN(odefile, x, s, p, ap, ntst, ncol,extravec,T,eps0,eps1);
    return
elseif ~strcmp(func2str(cds.curve),func2str(@homoclinicsaddlenode))
    [x,v] = init_LC_HSN(odefile, x, s, p, ap, ntst, ncol,extravec,T,eps0,eps1);
    return
end
...
% initialize homds
init_homds(odefile,x,p,ap,oldhomds.ntst,oldhomds.ncol,extravec,T,eps0,eps1,s);
...
%-----------------------------------------------------------------
function init_homds(odefile,x,p,ap,ntst, ncol,extravec,T,eps0,eps1,s)
global homds
oldhomds = homds;
homds = [];
