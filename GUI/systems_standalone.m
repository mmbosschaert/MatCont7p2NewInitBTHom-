...
% call external script to generate all the multilinear forms
% for the parameter-dependent normal form
if (exist('sym')==2 && gds.der(4,2)==1)
    system_name = gds.system;
    xx = cell2sym(strsplit(original_cor,','));
    pars = cell2sym(strsplit(original_pa,','));
    syms(xx)
    syms(pars)
    dxdt_cell = {};
    extra_coordinates = {};
    extra_equations_rhs = {};
    for i=1:size(original_equations,1)
        equation_split = strsplit( original_equations(i,:),'=');
        if size(equation_split,2) == 2
            if contains(equation_split{1},"'")
                dxdt_cell{end+1} = equation_split{2};
            else
                extra_coordinates{end+1} = strrep(strtrim(equation_split{1}),';','');
                extra_equations_rhs{end+1} = strrep(strtrim(equation_split{2}),';','');
            end
        end
    end
    % substitude extra equations into the differential equations manually
    for i=1:gds.dim
        for j=1:length(extra_coordinates)
            dxdt_cell{i} = strrep(dxdt_cell{i}, extra_coordinates{j}, ['(',extra_equations_rhs{j},')']);
        end
    end
    dxdt = str2sym(dxdt_cell);
    if gds.der(4,3) == 1
        order = 3;
    else
        order = 2;
    end
    generate_multilinear_forms(system_name, dxdt ,xx, pars, order, './Systems/');
else
    % remove old multilinearform file, if exist
    multilinarforms_file = ['./Systems/', gds.system, '_multilinearforms.m'];
    if exist(multilinarforms_file, 'file')
        delete(multilinarforms_file); 
    end
end
